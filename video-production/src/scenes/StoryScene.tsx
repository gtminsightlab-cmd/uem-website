...
const Panel = ({scene, kind}: {scene: VideoScene; kind: VisualKind}) => {
  ...
  if (kind === "compare") return <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, width: "100%"}}>{["Option A", "Option B"].map((label, index) => <div key={label} style={{background: "rgba(247,241,231,.08)", border: `1px solid ${accents[index]}88`, borderRadius: 20, padding: 24, transform: `translateY(${rise * (index + 1)}px)`}}><div style={{fontSize: 18, color: accents[index], fontWeight: 800, letterSpacing: 1}}>{label}</div>{[74, 52, 83].map((width, row) => <div key={row} style={{height: 9, marginTop: 22, borderRadius: 10, width: `${width - index * 5}%`, background: row === 2 ? accents[index] : "rgba(247,241,231,.28)"}} />)}</div>)}</div>;

  if (kind === "roles" || kind === "network") {
    const labels = scene.bullets.slice(0, 3);
    return <div style={{position: "relative", width: 430, height: 330, transform: `translateY(${rise}px)`}}><div style={{position: "absolute", inset: "120px 145px", borderRadius: 999, background: palette.lime, color: palette.ink, display: "grid", placeItems: "center", fontSize: 18, fontWeight: 900, boxShadow: `0 0 0 18px ${palette.lime}22`}}>Rx</div>{labels.map((label, index) => {const spots = [[0, 20], [250, 0], [130, 245]][index]; return <div key={label} style={{position: "absolute", left: spots[0], top: spots[1], width: 180, minHeight: 74, borderRadius: 18, background: "rgba(247,241,231,.09)", border: `1px solid ${accents[index]}88`, display: "grid", placeItems: "center", textAlign: "center", padding: 10, fontSize: 19, fontWeight: 700, color: palette.cream}}>{label}</div>;})}<svg width="430" height="330" style={{position: "absolute", inset: 0, zIndex: -1}}><path d="M180 145 L95 85 M250 145 L340 70 M220 205 L220 268" stroke={palette.mist} strokeOpacity=".35" strokeWidth="3" strokeDasharray="7 8" /></svg></div>;
  }

  if (kind === "flow") return <div style={{display: "flex", alignItems: "center", width: "100%", gap: 10, transform: `translateY(${rise}px)`}}>{scene.bullets.map((label, index) => <div key={label} style={{display: "contents"}}><div style={{width: 120, minHeight: 94, display: "grid", placeItems: "center", textAlign: "center", borderRadius: 18, padding: 12, color: palette.cream, background: "rgba(247,241,231,.08)", borderTop: `5px solid ${accents[index]}`, fontSize: 17, fontWeight: 700}}>{label}</div>{index < scene.bullets.length - 1 && <div style={{fontSize: 28, color: palette.lime}}>→</div>}</div>)}</div>;

  if (kind === "cta") return <div style={{width: "100%", padding: 34, borderRadius: 26, background: palette.cream, color: palette.ink, transform: `translateY(${rise}px)`, boxShadow: "0 24px 80px rgba(0,0,0,.28)"}}><div style={{fontSize: 17, textTransform: "uppercase", letterSpacing: 2, color: "#47716d", fontWeight: 800}}>Choose the right entry</div><div style={{fontSize: 37, fontFamily: "Georgia, serif", marginTop: 16, lineHeight: 1.08}}>See it. Test it. Shape what comes next.</div><div style={{display: "flex", gap: 12, marginTop: 25}}>{scene.bullets.map((b, i) => <span key={b} style={{borderRadius: 999, padding: "10px 14px", background: i === 0 ? palette.lime : palette.mist, fontWeight: 800, fontSize: 15}}>{b}</span>)}</div></div>;

  return <div style={{width: "100%", padding: 28, borderRadius: 24, background: "rgba(247,241,231,.08)", border: "1px solid rgba(247,241,231,.15)", transform: `translateY(${rise}px)`}}><div style={{display: "flex", alignItems: "end", gap: 12, height: 160}}>{bars.map((value, index) => <div key={index} style={{height: `${value * 100}%`, width: 72, borderRadius: "12px 12px 4px 4px", background: accents[index], transform: `scaleY(${interpolate(frame, [10 + index * 8, 42 + index * 8], [0, 1], {extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.out(Easing.cubic)})})`, transformOrigin: "bottom"}} />)}</div><div style={{display: "flex", gap: 10, marginTop: 22}}>{scene.bullets.map((b, i) => <span key={b} style={{fontSize: 14, borderRadius: 999, padding: "8px 11px", color: palette.cream, border: `1px solid ${accents[i]}99`}}>{b}</span>)}</div></div>;
};

export const StoryScene = ({story, scene, sceneIndex, durationInFrames}: {story: VideoStory; scene: VideoScene; sceneIndex: number; durationInFrames: number}) => {
